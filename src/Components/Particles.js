...
const ParticlesComponent = (props) => {
  ...
  useEffect(() => {
    setStartAnimation(true); // Start the animation on mount
  }, []);
  ...
  const options = useMemo(() => {
    ...
    return {
      ...
      interactivity: {
        ...
        modes: {
          push: {
            quantity: 50, // number of particles to add on click
          },
          repulse: {
  ...
  init={particlesInit}
  options={options}
  particles={startAnimation ? options.particles : []} // Conditionally enable particles based on startAnimation state
/>
};
